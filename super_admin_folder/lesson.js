// lesson.js v3 — super admin version
// Place this file at: super_admin_folder/lesson_folder/lesson.js
// This is identical in logic to the teacher's lesson.js because the
// create-module form (lessons.php) works the same for both roles.
// The only difference is the form action URLs in lessons.php which
// point to save_lessons_admin / save_assignment_admin / save_announcement_admin.
...
document.addEventListener("DOMContentLoaded", function () {
    ...
    function buildActivityQuestion(modIdx, lesIdx, aIdx, qCount) {
        ...
        sel.addEventListener("change", () => {
            if (sel.value === "multiple_choice") { essay.style.display = "none"; mc.style.display = "block"; }
            else { essay.style.display = "block"; mc.style.display = "none"; }
        });
        return q;
    }
    ...
    if (cwContainer && addCwBtn) {
        function reNumberAssignments() {
            cwContainer.querySelectorAll(".cw-assignment-card").forEach((card, i) => {
                card.querySelector(".cw-assignment-num").textContent = "Assignment " + (i + 1);
            });
            const emptyEl = document.getElementById("cwAssignmentEmpty");
            if (emptyEl) emptyEl.style.display =
                cwContainer.querySelectorAll(".cw-assignment-card").length === 0 ? "flex" : "none";
        }
        ...
        function addAssignmentCard() {
            ...
            fileInput.addEventListener("change", () => {
                ...
                uploadBox.style.borderColor = "#00C950";
            });
            removeFile.addEventListener("click", () => {
                fileInput.value = "";
                fileNameLbl.textContent = "No file chosen.";
                previewItem.style.display = "none";
                uploadBox.style.borderColor = "";
            });
            card.querySelector(".cw-remove-btn").addEventListener("click", () => {
                card.remove();
                reNumberAssignments();
            });
    ...
    if (annContainer && addAnnBtn) {
        ...
        function reNumberAnnouncements() {
            ...
            if (emptyEl) emptyEl.style.display =
                annContainer.querySelectorAll(".ann-card").length === 0 ? "flex" : "none";
        }
